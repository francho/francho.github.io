...
description: "Create a new book review entry under src/i-like/libros. Use when asked to add, create, or write a book review (reseña de libro). Fetches missing metadata (ISBN, author, cover art) from Open Library, downloads and optimizes the cover to WebP, applies minimal orthographic corrections to the Spanish review text, and creates the MDX file with correct frontmatter. Invokes the openlibrary and download-book-cover skills automatically."
argument-hint: "Book title and review text"
user-invocable: true
---
...
- Adding a new book review to `src/i-like/libros/`.
- Creating a new book entry with cover image and Open Library metadata.
- The user provides a book title and a review written in Spanish.

## Inputs

- Book title (required if not already provided).
- Full review text in Spanish (required if not already provided).

## Step-by-Step Workflow

Use tool: `openlibrary` to fetch metadata and `download-book-cover` to resolve, download, convert, and place the cover asset.
...
6. Once the book match is confirmed, run these tasks in parallel:
   - Invoke tool: `download-book-cover` with the book title as a delegated subtask to create the cover file at `src/i-like/libros/<slug>.webp`.
...
- Cover acquisition and placement must use tool: `download-book-cover`.
